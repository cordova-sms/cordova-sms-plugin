...
public class Sms extends CordovaPlugin {
    ...
    private boolean getTextsAfterTimeStamp(CallbackContext callbackContext, String phoneNumber, JSONArray args, JSONObject result) throws JSONException {
        ...
        if (!isNumeric(timeStamp)) {
            ...
            result.put("error", String.format("Time stamp provided (%s) is non-numeric.", timeStamp));
            callbackContext.success(result);
            return false;
        ...
        Log.d(TAG, String.format("Querying for number %s with texts received after %s time stamp.", phoneNumber, timeStamp));
    ...
    private boolean getTexts(JSONArray args, CallbackContext callbackContext, JSONObject result, String phoneNumber) throws JSONException {
        ...
        if (args.length() >= 2) { // We want numberOfTexts to be the second one
            ...
            try {
                ...
                String errorMessage = String.format("Input provided (%s) is not a number", args.getString(1));
    ...
    private JSONArray readTextsFrom(String numberToCheck, Integer numberOfTexts) throws JSONException {
        ContentResolver contentResolver = cordova.getActivity().getContentResolver();
        String[] smsNo = new String[] {
            numberToCheck
        };
        String sortOrder = "date DESC" + ((numberOfTexts == READ_ALL) ? "" : " limit " + numberOfTexts);
        Cursor cursor = contentResolver.query(Uri.parse("content://sms/inbox"), null, "address=?", smsNo, sortOrder);
        ...
        while (cursor.moveToNext()) {
            ...
            try {
                ...
                Log.d(TAG, "time: " + cursor.getString(cursor.getColumnIndex("date")) + " message: " + cursor.getString(cursor.getColumnIndex("body")));
    ...
    private JSONArray readTextsAfter(String numberToCheck, String timeStamp) throws JSONException {
        ContentResolver contentResolver = cordova.getActivity().getContentResolver();
        ...
        Cursor cursor = contentResolver.query(Uri.parse("content://sms/inbox"), null, "address=? AND date>=?", queryData, sortOrder);
        JSONArray results = new JSONArray();
        while (cursor.moveToNext()) {
            JSONObject current = new JSONObject();
            try {
